...
const DoctorsResult = ({ selectedSpeciality, setSelectedDoc, selectedDoc }: Props) => {
  ...
  return (
    <div>
        ...
        <tbody>
          {filteredDoctors.map((doctor, index) => (
            ...
            >
              ...
              <td>
                <svg onClick={() => setIsOpen(true)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.0} stroke="currentColor" className="w-6 h-6 hover:fill-primary transition-all duration-500">
      ...
      {isOpen && (
        <div onClick={handleOutsideClick} className="backdrop-blur-sm bg-white/30 main-modal fixed w-full h-100 inset-0 z-50 overflow-hidden flex justify-center items-center animated fadeIn faster">
          <div
            className="border border-teal-500 bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
            <div className="modal-content py-4 text-left px-6">
              ...
              <div className="flex justify-end pt-2">
                <button onClick={closeModal} className="focus:outline-none px-4 bg-teal-500 p-3 ml-3 rounded-lg text-white hover:bg-teal-400">
                  Close
                </button>
              </div>
            </div>
